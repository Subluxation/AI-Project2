 ...
 * @author amy
 */
public class CustomMove extends AbstractAction {
	/**
	 ...
	 */
	public CustomMove(Toroidal2DPhysics space, Position currentLocation, 
		...
		KvRotational = 4 * 2.53;
		KpRotational = 4 * 1.6;
		KvTranslational = 4 * 0.56f;
		KpTranslational = 4 * 0.08f;
	}
...
//		float Kp = 0.16f;
	...
	public Vector2D pdControlMoveToGoal(Toroidal2DPhysics space, Position goalLoc,
		...
		double velocityErrorX = 0.1 * (goalVelocity.getXValue() - currentLoc.getTranslationalVelocityX());
		double velocityErrorY = 0.1 * (goalVelocity.getYValue() - currentLoc.getTranslationalVelocityY());
