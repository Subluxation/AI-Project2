...
/**
 * This collector performs actions using the A * algorithm.
 * If the ship has low energy, get a beacon
 * If the ship is low on resources, get resources
 * If the ship had enough resources, go home and get asteroids along the way
 * 
...
public class HillClimbTeamClient extends TeamClient {
	...
	private AbstractAction getAggressiveAsteroidCollectorAction(Toroidal2DPhysics space, Ship ship) {
		...
		// Update path and grid every 25 timesteps
		if (space.getCurrentTimestep() % 25 == 0 || space.getCurrentTimestep() == 0) {	
			...
			else if (ship.getResources().getTotal() > 500)
			{
				Base base = findNearestBase(space, ship);
				// If the base is farther than the nearest asteroid
				// Idea is to collect asteroids along the way to the base
				if (space.findShortestDistance(ship.getPosition(), base.getPosition()) > space.findShortestDistance(ship.getPosition(), pickNearestFreeAsteroid(space, ship).getPosition()))
				{
...
//			this.path = HillClimb(space, goal, ship);
	...
	public void initialize(Toroidal2DPhysics space) {
		...
		double widthGrid = space.getWidth() / 15;
		double heightGrid = space.getHeight() / 15;
