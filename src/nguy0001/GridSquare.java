	...
	public ArrayList<GridSquare> getAdjacent(ArrayList<ArrayList<GridSquare>> grid)
	{
		...
		for (int i = 0; i < grid.size(); i++)
		{
			for (int j = 0; j < grid.size(); j++)
			{
				...
				if (grid.get(i).get(j).center == this.center)
				{
					row = i;
					col = j;
				}		
		...
		{
			// SW row + 1, col - 1
//			if (grid.get(row + 1).get(col - 1).isEmpty)
				adjacentGrids.add(grid.get(row + 1).get(col - 1));
			
			// S row + 1
//			if (grid.get(row + 1).get(col).isEmpty)
...
//				if (grid.get(row - 1 + grid.size()).get(col - 1).isEmpty)
					adjacentGrids.add(grid.get(row - 1 + grid.size()).get(col - 1));
				
				// N row - 1
				// add 25 to make it wrap around
//				if (grid.get(row - 1 + grid.size()).get(col).isEmpty)
...
//				if (grid.get(row - 1 + grid.size()).get(col + 1).isEmpty)
			...
			else if (col == 0)
			{
				// SW row + 1, col - 1
				// add 25 to make it wrap around
//				if (grid.get(row + 1).get(col - 1 + grid.size()).isEmpty)
...
//				if (grid.get(row + 1).get(col + 1).isEmpty)
				...
				// W col - 1
				// add 25 to make it wrap around
//				if (grid.get(row).get(col - 1 + grid.size()).isEmpty)
...
//				if (grid.get(row).get(col + 1).isEmpty)
				...
				// NW row - 1, col - 1
				// add 25 to make it wrap around
//				if (grid.get(row - 1 + grid.size()).get(col - 1 + grid.size()).isEmpty)
...
//		for (GridSquare sq: adjacentGrids)
//		{
//			if (!sq.isEmpty && !sq.containsGoal)
//				adjacentGrids.remove(sq);
//		}
